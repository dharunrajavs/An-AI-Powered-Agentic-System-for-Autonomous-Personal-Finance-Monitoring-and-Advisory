...
import { z } from 'zod';
import { useAddTransaction, useUpiAccounts } from '../../hooks';
    ...
    addTransaction.mutate(
      {
        ...
        account: values.paymentMethod === 'upi' && upiAccounts.length > 0 ? upiAccounts[0].upiId : values.account,
              ...
              {(['upi', 'cash'] as const).map((method) => {
                ...
                return (
                  <Pressable
                    ...
                    className={`flex-1 flex-row items-center justify-center gap-1.5 py-2.5 rounded-xl border ${active ? 'bg-primary border-primary' : 'bg-surface-container-low border-outline-variant/30'}`}
                  >
