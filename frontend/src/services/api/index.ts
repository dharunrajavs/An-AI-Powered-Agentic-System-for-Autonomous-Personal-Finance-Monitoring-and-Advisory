...
function mapTransaction(row: any): Transaction {
  return {
    ...
    messageHash: row.message_hash ?? undefined,
  };
}
...
async function adjustAccountBalance(accountName: string, delta: number): Promise<void> {
  if (!accountName) return;
  const uid = await getUserId();
  const { data, error } = await supabase
    .from('connected_accounts')
    .select('id, balance, nickname, institution')
    .eq('user_id', uid);
  if (error || !data) return;
  const match = data.find(
    (a) =>
      String(a.nickname).toLowerCase() === accountName.toLowerCase() ||
      String(a.institution).toLowerCase() === accountName.toLowerCase(),
  );
  if (!match) return;
  const newBalance = Math.round((Number(match.balance) + delta) * 100) / 100;
  await supabase.from('connected_accounts').update({ balance: newBalance }).eq('id', match.id).eq('user_id', uid);
}
...
export async function deleteTransaction(id: string): Promise<{ success: true }> {
  ...
  const { error } = await supabase
    ...
    .eq('user_id', uid);
  if (error) throw error;
  if (existing) await adjustAccountBalance(existing.account ?? '', -(Number(existing.amount) || 0));
  return { success: true };
}
...
export async function updateTransaction(
  ...
  if (existing && patch.amount !== undefined && Number(patch.amount) !== Number(existing.amount)) {
    await adjustAccountBalance(existing.account ?? '', Number(patch.amount) - Number(existing.amount));
  } else if (existing && patch.account !== undefined && patch.account !== existing.account) {
    await adjustAccountBalance(existing.account ?? '', -(Number(existing.amount) || 0));
    await adjustAccountBalance(patch.account, Number(existing.amount) || 0);
  }
...
export async function addTransaction(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      message_hash: input.messageHash ?? null,
    })
    .select()
    .single();
  if (error) throw error;
  await adjustAccountBalance(input.account ?? '', Number(input.amount) || 0);
  return mapTransaction(data);
}

// ─── SMS-detected transactions ──────────────────────────────────

export async function getTransactionByMessageHash(messageHash: string): Promise<Transaction | null> {
...
export async function saveDetectedSmsTransactions(
  ...
  for (const item of parsed) {
    ...
    const existing = await getTransactionByMessageHash(item.messageHash).catch(() => null);
    if (existing) {
      skipped += 1;
      continue;
    }
    ...
    const { error } = await supabase.from('transactions').insert({
      ...
      payment_method: item.paymentMethod ?? 'unknown',
      notes: null,
      flagged: false,
      source: 'sms',
      reference_id: item.referenceId ?? null,
      bank_name: item.bankName,
      account_last4: item.accountLast4 ?? null,
      message_hash: item.messageHash,
    });
...
export async function getBudgets(): Promise<Budget[]> {
  ...
  const { data, error } = await supabase
    ...
    .eq('user_id', uid);
  if (error) throw error;
  return applyBudgetLimitOverrides((data ?? []).map(mapBudget));
}
