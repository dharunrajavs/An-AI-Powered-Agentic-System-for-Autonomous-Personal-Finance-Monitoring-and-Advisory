...
import { getConnectedAccounts as apiGetConnectedAccounts, linkAccount as apiLinkAccount, unlinkAccount as apiUnlinkAccount } from '../api';
...
export function linkAccount(input: Omit<ConnectedAccount, 'id' | 'syncStatus'>): Promise<ConnectedAccount> {
  if (!USE_MOCK) return apiLinkAccount(input);
  const created: ConnectedAccount = { ...input, id: `acct_${Date.now()}`, syncStatus: 'synced' };
  accounts = [...accounts, created];
  return delay(created, 600);
}

export function adjustMockAccountBalance(accountName: string, delta: number): void {
  const match = accounts.find(
    (a) =>
      a.nickname.toLowerCase() === accountName.toLowerCase() ||
      a.institution.toLowerCase() === accountName.toLowerCase(),
  );
  if (match) match.balance = Math.round((match.balance + delta) * 100) / 100;
}
